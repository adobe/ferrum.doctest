...
const { assertEquals: ckEq, pipe, extend, takeUntil, list, append, type, reject, map } = require('ferrum');
const { isRoot, liftArray, transformStringList, findPkg, rmRecursiveSync } = require('ferrum.doctest/cli');
...
it('liftArray', () => {
  const nop = (v) => ckIs(v, liftArray(v));
  const wrp = (v) => {
    const w = liftArray(v);
    ckIs(type(w), Array);
    ckIs(w.length, 1);
